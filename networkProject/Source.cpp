...
const string trainDataPath = "D:\\Study\\special\\neuralNetwork\\Project1\\Project1\\train.txt";
const string testDataPath = "D:\\Study\\special\\neuralNetwork\\Project1\\Project1\\test.txt";
const string weightsPath = "D:\\Study\\special\\neuralNetwork\\Project1\\Project1\\weights.txt"; 
...
public:
	...
	void matrixMultiply(int M, int N, int K, const float* A, const float* B, float* C)
	{
		for (int i = 0; i < M; ++i)
		{
			float* c = C + i * N;
	...
	void matrixMultiplySum(int M, int N, int K, const float* A, const float* B, float* C)
	{
		for (int i = 0; i < M; ++i)
		{
			float* c = C + i * N;
	...
	void forwardFeed(float* input_data) {
		...
		for (int i = 0; i < layersN - 1; i++) {
			...
			matrixMultiply(1, arch[i + 1], arch[i], a, b, c);
	...
	void backPropogation(float* rightResults, float lr) {
		...
		for (int i = layersN - 1; i > 0; i--) {
			...
			matrixMultiplySum(arch[i - 1], arch[i], 1, a, b, c);
